...
window.onload = function init()
{
	...
	ele.oninput = ele.onchange = function (event) {
  	...
  	var ele = document.getElementById("rotYcam");
	ele.oninput = ele.onchange = function (event) {
		var rotYcam = (event.srcElement || event.target).value;
		document.getElementById("directionvalCam").innerHTML =document.getElementById("rotYcam").value;
   };
...
function render()
{
	...
	var degrees = document.getElementById("rotYcam").value;
	if(degrees == undefined){
		degrees = 1.0;
	}
	var x = eye[0]*Math.cos((degrees*3.1415926)/180) - eye[2]*Math.sin((degrees*3.1415926)/180);
	var z = eye[0]*Math.cos((degrees*3.1415926)/180) + eye[1]*Math.sin((degrees*3.1415926)/180);
	//Set initial view
	 eye = vec3(x, 1.0, 6+z);
	 at =  vec3(x, 1.0, z);
	 up =  vec3(0.0, 1.0, 0.0);
